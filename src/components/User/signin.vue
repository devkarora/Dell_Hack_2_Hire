        ...
        <v-form @submit.prevent="resetPassword" ref="form" v-model="respassvalid">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn @click="passwordReset=false" flat :disabled="loading" color="error" raised ripple>Close </v-btn>
                </v-card-actions>
            </v-card>
        </v-form>
    </v-dialog>
                        ...
                        <v-layout row wrap>
                            <v-flex xs12>
                                <div class="text-xs-right">
                                    <v-btn @click="clear()" round large :disabled="loading" color="error" raised ripple>
                                        <v-icon class="mr-2">clear</v-icon>
                                        Clear
                                    </v-btn>
                                    <v-btn :disabled="!valid || loading" round large color="success" :loading="loading" type="submit" raised ripple>
                                        <span slot="loader" class="custom-loader">
                                            <v-icon light>cached</v-icon>                                 
                                        </span>
                                        <v-icon class="mr-2">near_me</v-icon>
                                        Log In
                                    </v-btn>
                                </div>                                
                            </v-flex>                            
